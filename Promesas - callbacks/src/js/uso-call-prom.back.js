...
Promise.all([buscarHeroe(heroeId), buscarHeroe(heroeId2)])
  ...
  /**
   * Si el "heroeId" y el "heroeId2" llegan a dar errores en sus
   * variables el "catch" se ejecuta en lo que tiene en la clase
   * promesas.js y como una de sus variables no encuentran nada en el
   * objeto posterior se ejecuta el "reject"
   */

  //EJEMPLO PARA USAR "reject"
  .catch((err) => {
    alert(err);
  })
  .finally(() => {
